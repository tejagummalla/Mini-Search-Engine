...
def get_term_frequency ():
    ...
    for doc in doc_len:
        for term in inv_ind:
            if doc in inv_ind[term]:
                if doc in doc_model:
                    doc_model[doc]+=math.pow(inv_ind[term][doc]*idf[term]/int(doc_len[doc]),2)
                else:
                    doc_model[doc]=math.pow(inv_ind[term][doc]*idf[term]/int(doc_len[doc]),2)
    ...
    with open("Run2-Cosine-PR.txt","w") as tf:
        for var in que:
            print var
            do_array={}
            sum_idf=0
            ...
            for doc in do_array :
                ...
                for item in tf_q:
                    if item in inv_ind and doc in inv_ind[item]:
                        numerator+= tf_q[item]*idf[item]*inv_ind[item][doc]/int(doc_len[doc])
