...
def bm25():
    ...
    with open("inverted_index_stop.txt", "r") as f:
        ...
        for line in f:
            ...
            inv_ind[word[0]] = lis
    tok = 0
    with open("doc_length_stop.txt", "r") as f1:
        doc_length = {}
        for line in f1:
            doc = line.split()
    ...
    for qid in query_ind.keys():
        ...
        for val in sorted_bm:
            if (i < 101):
                ...
                print doc_type
                #run3.append(str(qid) + " "+ str(i)+" "  + str(val[0]) + " " + str(val[1]) + " " + doc_type+" "+ str(precision)+" "+str(recall))
                run3.append(str(qid) + " " + "Q0 " + str(val[0]) + " " + str(i) + " " + str(val[1]) + " BM25_STOP "+ doc_type+" "+ str(precision)+" "+str(recall))
                i = i + 1
            else:
                break
    with open("bm25stoprun.txt", "w+") as f5:
        for v in run3:
            f5.write(v)
            f5.write("\n")
    f5.close()
